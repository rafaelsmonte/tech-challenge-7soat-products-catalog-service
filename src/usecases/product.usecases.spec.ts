...
describe('ProductUseCases', () => {
  ...
  it('should find all products with their category and stock', async () => {
    ...
    expect(result).toEqual([{ product, category, stock }]);
  });
  ...
  it('should throw CategoryNotFoundError when a product has no category', async () => {
    ...
    stockGateway.findByProductId.mockResolvedValue(
      new Stock(1, new Date(), new Date(), 1, 50),
    );

    await expect(
      ProductUseCases.findAll(productGateway, categoryGateway, stockGateway),
  ...
  it('should find a product by ID with its category and stock', async () => {
    ...
    const result = await ProductUseCases.findById(
      productGateway,
      categoryGateway,
      stockGateway,
      1,
  ...
  it('should delete a product', async () => {
    ...
    await expect(
      ProductUseCases.delete(productGateway, 1),
  ...
  it('should throw ProductNotFoundError when deleting a non-existent product', async () => {
    ...
    await expect(ProductUseCases.delete(productGateway, 1)).rejects.toThrow(
